...
明显头重。仅 execute-command 就占全部调用的 73.4%，file write/edit 占 10.4%、file
read/search 占 9.3%；agent/task（1.5%）和 web/remote/lookup（1.0%）是细薄的切片，
其余一切都被折叠进 `Other`（4.3%）。因此一旦把提供商特定的工具名归一化为共享
...
标注了平均每次调用秒数）。Execute-command 仍以约 1,826h 领先；agent/task 平均每次
有效调用约 43s（总计 128h），web/remote/lookup 平均约 16s（总计 33.5h）。
`Other` 桶以约 968h（平均约 109s）的耗时远超其 4.3% 的调用占比。
File read/search 尽管是被调用第三多的类别，却仅以平均约 0.93s 花费约 17.8h。
...
是其**总延迟占比**，二者颠倒。`<1s` 分箱占 61.3% 的调用却仅占
0.6% 的总延迟，而 `1–10s` 又添了 25.4% 的调用，却只占 4.2% 的延迟。在另一
端，`>1m` 分箱仅占 3.4% 的调用，却占全部工具延迟的 83.6%；`10s–1m`（10.0% 的调用）
贡献 11.7%。因此少数几次慢调用占据了花在工具上时间的
