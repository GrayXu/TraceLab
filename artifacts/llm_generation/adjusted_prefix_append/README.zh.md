...
- **汇总 CSV 是精确的（全量数据）。** 各组的分位数、中位数、min、max 都在**每一个**对上计算，而非样本。分位数采用线性插值（`(n−1)·q`），与 `np.percentile` 的默认行为一致。
...
- `write_summary_csv(...)` — 各 `provider`/`metric` 的分位数 CSV，使用共享的 `median`/`percentile`/`fmt` 辅助函数。
...
- 减去上一步输出会把追加点云相对于原始追加往下拉：每个步骤看似"新"输入的很大一部分其实是上一个 assistant 响应被重放，而非外部内容。对 Claude，原始追加的中位数为 967 tokens，去除上一步输出后为 285。汇总 CSV 量化了这一差距（`raw_append` vs `adjusted_append`，以及减法把追加减到零的 `clipped_after_subtract` 占比）。
