#!/usr/bin/env python3
"""Extract privacy-sanitizable executable facts from command tool inputs.
...
Precision over recall / self-validating: we only claim ``success`` when the parse is clean.
A parse error (``root.has_error`` — e.g. a truncated command, or Codex's ``apply_patch`` patch body
which is data, not shell) produces an empty executable list rather than a guessed label.
...
COMMAND_TOOL_NAMES = frozenset({"Bash", "exec_command", "shell", "shell_command"})
...
def _parser():
    ...
    if _PARSER is None:
        try:
            ...
            raise RuntimeError(
                "tree-sitter and tree-sitter-bash are required for public-trace sanitization "
                f"(import failed: {exc})"
            ) from exc
...
def _parse_status(labels: list[str], not_sure: int) -> str:
    """Return ``success``, ``partial``, or ``failed`` for one extraction."""
    if not_sure == 0:
        return "success"
    return "partial" if labels else "failed"


def extract_executable_facts(raw: Any) -> dict[str, Any]:
    """Return the four executable fields stored by the public-trace sanitizer.

    This function intentionally returns private executable names. The sanitizer must transform
    ``executables`` and every executable token in ``command_skeleton`` before publishing them.
    """
    result = _extract(raw)
    reason = result["reason"]
    if isinstance(reason, str) and reason.startswith("partial:"):
        reason = reason.removeprefix("partial:")
    return {
        "executables": result["labels"],
        "executable_parse_status": _parse_status(result["labels"], result["not_sure"]),
        "executable_parse_reason": reason,
        "command_skeleton": result["command_skeleton"],
    }
